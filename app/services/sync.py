        ...
        async with AsyncSessionLocal() as db:
            ...
            for product_data in normalized_products:
                ...
                result = await db.execute(
                    select(Product).where(Product.external_id == product_data.external_id)
                )
                ...
                if existing_product:
                    ...
                    existing_product.height = product_data.height
                    existing_product.length = product_data.length
                    existing_product.depth = product_data.depth
                    updated_count += 1
                else:
                    # Create new product
                    new_product = Product(
